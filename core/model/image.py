...
class ImageLanguageModel(models.Model):
        ...
        if self.meta is not None:
            if len(self.meta) == 0:
                if self.info is not None:
                    ...
                    self.info = ''
        self.meta = ''
        super(ImageLanguageModel, self).save(*args, **kwargs)


class ImageModel(BaseModel, Preferences):
    path = str(datetime.now().year) + '/' + str(datetime.now().month)
    fullpath = 'images/galleries/images/' + path
