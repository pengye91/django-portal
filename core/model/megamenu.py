...
class MegaMenuItemLanguageModel(models.Model):
        ...
        if self.meta is not None:
            if len(self.meta) == 0:
                if self.info is not None:
                    ...
                    self.info = ''
        self.meta = ''
        super(MegaMenuItemLanguageModel, self).save(*args, **kwargs)


class MegaMenuItemModel(BaseModel, Preferences):
    path = str(datetime.now().year) + '/' + str(datetime.now().month)
    fullpath = 'images/megamenu/' + path
    ...
    def __init__(self, *args, **kwargs):
        super(MegaMenuItemModel, self).__init__(*args, **kwargs)
