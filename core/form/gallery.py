...
from core.models import Gallery, GalleryLanguage, Language, PermisionGroup
...
class AdmItemForm(forms.ModelForm):
    ...
    id = forms.ModelChoiceField(queryset=Gallery.objects.all(), widget=forms.HiddenInput())
    ...
    parent = forms.ModelChoiceField(queryset=Gallery.objects.all())
    tags = TagField(required=False)

    class Meta:
        model = Gallery
        fields = ('id', 'temp', 'date', 'parent', 'image', 'sites', 'active', 'tags', 'permissions')

    def __init__(self, *args, **kwargs):
        super(AdmItemForm, self).__init__(*args, **kwargs)
        try:
            root_pgroup = PermisionGroup.objects.filter(model='Gallery')
        except Exception, e:
            print 'form/gallery.py: ', e
            root_pgroup = None
...
class AdmItemLanguageForm(forms.ModelForm):
    id = forms.ModelChoiceField(queryset=GalleryLanguage.objects.all(), widget=forms.HiddenInput())
    temp = forms.BooleanField(widget=forms.HiddenInput(), required=False)
