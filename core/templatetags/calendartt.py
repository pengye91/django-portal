...
def do_event_calendar(parser, token):
    ...
    try:
        ...
        print 'core/templatetags, calendartt.py: ', e
    return EventCalendarNode(year, month, event_list)

def do_event_calendar_by_date(parser, token):
    year = token.strftime('%Y')
    month = token.strftime('%m')
...
register.tag("event_calendar_by_date", do_event_calendar_by_date)
