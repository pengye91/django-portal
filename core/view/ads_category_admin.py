...
from core.models import AdCategory, AdCategoryLanguage
from core.form.ads import AdsCatItemForm as AdmItemForm
from core.form.ads import AdsCatItemLanguageForm as AdmItemLanguageForm

class SystemObject(SystemManager):

    def __init__(self, request, *args, **kwargs):
        super(SystemObject, self).__init__(request, *args, **kwargs)
        self.urls.add = 'core.view.ads_category_admin.add_item'
        self.urls.edit = 'core.view.ads_category_admin.edit_item'
        self.urls.show_items = 'core.view.ads_category_admin.show_items'
        ...
        self.manager.model = AdCategory()
        self.manager.modelLanguage = AdCategoryLanguage()
        ...
        self.manager.debugger.filename = 'ads_category_admin.py'
        self.manager.moduleName = '__adm_Ads_categories__'
        self.data.update({ 'filter_activity': reverse('core.view.ads_category_admin.show_items') })
        toppanel = {}
...
def show_items(request):
    ...
    system.template = loader.get_template(system.sheet.get_sheet_file('admin_ads_categories_list'))
...
def edit_item(request, itemId):
    ...
    system.template = loader.get_template(system.sheet.get_sheet_file('admin_ads_categories_edit'))
    c = RequestContext(request, system.get_context())
    return HttpResponse(system.template.render(c))
...
def add_item(request):
    ...
    if system.permission.user is None:
        return HttpResponseRedirect(reverse('core.view.userprofileadmin.login'))
    system.manager.new()
    return HttpResponseRedirect(reverse('core.view.ads_category_admin.edit_item', args=(system.manager.item.id,)))
